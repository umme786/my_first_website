import React from 'react';
import { FaFacebookF, FaTwitter, FaPinterest, FaInstagram, FaPlay } from "react-icons/fa";
const Banner = () => {
  const [state] = React.useState({title: 'I am Umme Ruman', text: 'I am Umme, professional web developer with 1.5 years of experience in this field​', image: "/images/k-couple.png"});
  return (
    <header className="header">
      <div className="container">
        <div className="row">
          <div className="col-6 margin-top">
            <div className="header__content">
              <div className="header__section">
                <ul className="header__ul">
                  <li> <FaFacebookF /> </li>
                  <li> <FaTwitter /> </li>
                  <li> <FaPinterest /> </li>
                  <li> <FaInstagram /> </li>
                </ul>
                {/* <h1> I am Jonathan Doe </h1> */}
                <h1> {state.title} </h1>
                {/* <p> I,m Jonathan, professional web developer with long time experience in this field​ </p> */}
                <p> {state.text} </p>
                <div className="header__buttons">
                  <a href="" className="btn btn-outline"> My Portfolio </a>
                  &nbsp; &nbsp; &nbsp;
                  <a href="" className="btn btn-smart"> <FaPlay className="play" /> </a>
                </div>
              </div>
            </div>
          </div>
          <div className="col-6">
            <div className="banner__img">
              {/* <img src="/images/man-01.png" alt="man" /> */}
              <img src={state.image} alt="man" />
            </div>
          </div>
        </div>
      </div>
    </header>
  );
};

export default Banner
